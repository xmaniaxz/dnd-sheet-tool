import { databases, DATABASE_ID, CHARACTERS_COLLECTION_ID, ID, Query, account } from './appwrite';
...
export const characterService = {
   ...
   */
  async create(character: CharacterData, teamId?: string): Promise<CharacterDocument> {
    try {
      ...
      // Prepare the document - Appwrite will auto-generate $id if not provided
      const doc = {
        ...character,
        userId,
  ...
  async get(documentId: string): Promise<CharacterDocument | null> {
    try {
      ...
      if (doc.userId !== userId && doc.teamId !== currentTeamId) {
        console.warn('Access denied: user does not have access to this character');
        return null;
      }
   ...
   */
  async update(documentId: string, character: Partial<CharacterData>): Promise<CharacterDocument> {
    try {
      const userId = await getCurrentUserId();
      if (!userId) {
      ...
      const doc: any = { ...character };

      // Convert complex objects to JSON strings
      if (character.identity) doc.identity = JSON.stringify(character.identity);
      if (character.abilities) doc.abilities = JSON.stringify(character.abilities);
      if (character.proficiencies) doc.proficiencies = JSON.stringify(character.proficiencies);
      if (character.hp) doc.hp = JSON.stringify(character.hp);
      if (character.deathSaves) doc.deathSaves = JSON.stringify(character.deathSaves);
      ...
      if (character.inventory) doc.inventory = JSON.stringify(character.inventory);
   ...
   */
  async list(options: { teamId?: string; limit?: number } = {}): Promise<CharacterDocument[]> {
    try {
      const userId = await getCurrentUserId();
      if (!userId) {
      ...
      return response.documents.map(parseCharacterDocument);
    } catch (error) {
...
function parseCharacterDocument(doc: any): CharacterDocument {
  return {
    ...
    name: doc.name,
    level: doc.level,
    identity: typeof doc.identity === 'string' ? JSON.parse(doc.identity) : doc.identity,
    ...
    proficiencies: typeof doc.proficiencies === 'string' ? JSON.parse(doc.proficiencies) : doc.proficiencies,
    ac: doc.ac,
    ...
    deathSaves: typeof doc.deathSaves === 'string' ? JSON.parse(doc.deathSaves) : doc.deathSaves,
    hitDice: typeof doc.hitDice === 'string' ? JSON.parse(doc.hitDice) : doc.hitDice,
    feats: typeof doc.feats === 'string' ? JSON.parse(doc.feats) : doc.feats,
    languages: doc.languages,
    notes: doc.notes,
    inventory: typeof doc.inventory === 'string' ? JSON.parse(doc.inventory) : doc.inventory,
    spells: typeof doc.spells === 'string' ? JSON.parse(doc.spells) : doc.spells,
  };
}
